"""
OW Hero-vs-Hero Modifier Switchboard (0/1)

What it does
- Pick an "player" hero on the left
- For that player, set each "follower" hero to 1, 2, or 3
- Click Compile to export:
  1) A flattened table (player-major order): idx = playerIndex * N + followerIndex
  2) A per-player matrix (dict of dict)
- Save/Load to JSON
- Optional: Edit hero list in-app (or load from a text file)

Notes
- This tool stores only 0/1 as requested.
- You can later decode 0/1 -> {1.00, 1.50}
...
class SwitchboardApp(ttk.Frame):
    def __init__(self, root):
        ...
        # Data: table[player_index][follower_index] = 0/1
        # Default everything to 2
        self.table = [[0 for _ in range(self.N)] for __ in range(self.N)]
        ...
        self._build_style()
        self._build_layout()
        self._render_follower_rows()

        self._select_player(0)
    ...
    def _build_layout(self):
        self.root.title("OW Hero Switchboard (0/1)")
        ...
        ttk.Label(left, text="player Hero", style="Header.TLabel").grid(row=0, column=0, sticky="w")

        ttk.Button(left, text="Edit Hero List…", command=self._open_hero_editor).grid(row=1, column=0, sticky="we", pady=(6, 6))

        self.player_list = tk.Listbox(left, height=22, exportselection=False)
        self.player_list.configure(background="#1e1e1e",    foreground="#d4d4d4",    selectbackground="#444444",    selectforeground="#ffffff",    highlightthickness=0)
        self.player_list.grid(row=2, column=0, sticky="nsew")
        self.player_list.bind("<<ListboxSelect>>", self._on_player_select)

        for h in self.heroes:
            self.player_list.insert("end", h)

        # Right: follower grid + controls
        right = ttk.Frame(self)
        right.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        right.grid_rowconfigure(2, weight=1)
        right.grid_columnconfigure(0, weight=1)

        self.player_title = ttk.Label(right, text="follower Modifiers", style="Header.TLabel")
        self.player_title.grid(row=0, column=0, sticky="w")

        controls = ttk.Frame(right)
        controls.grid(row=1, column=0, sticky="we", pady=(6, 8))
        controls.grid_columnconfigure(6, weight=1)

        ttk.Label(controls, text="Quick Fill for current player:", style="Small.TLabel").grid(row=0, column=0, sticky="w")

        ttk.Button(controls, text="Set All = 0", command=lambda: self._fill_player_row(0)).grid(row=0, column=1, padx=4)
        ttk.Button(controls, text="Set All = 1", command=lambda: self._fill_player_row(1)).grid(row=0, column=2, padx=4)
        ...
        self.follower_frame = ScrollableFrame(right)
        self.follower_frame.grid(row=2, column=0, sticky="nsew")
    ...
    def _render_follower_rows(self):
        # Clear existing follower rows
        for w in self.follower_frame.inner.winfo_children():
            w.destroy()

        header = ttk.Frame(self.follower_frame.inner)
        header.grid(row=0, column=0, sticky="we", padx=6, pady=(6, 4))
        header.grid_columnconfigure(0, weight=1)

        ttk.Label(header, text="follower Hero", style="Small.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(header, text="Value (0/1)", style="Small.TLabel").grid(row=0, column=1, sticky="e")

        self.follower_vars = []
        for i, follower in enumerate(self.heroes):
            row = ttk.Frame(self.follower_frame.inner)
            row.grid(row=i + 1, column=0, sticky="we", padx=6, pady=2)
            row.grid_columnconfigure(0, weight=1)

            ttk.Label(row, text=follower).grid(row=0, column=0, sticky="w")

            var = tk.StringVar(value="0")
            self.follower_vars.append(var)

            combo = ttk.Combobox(row, textvariable=var, values=("0", "1"), width=4, state="readonly")
            combo.configure(background="#2a2a2a", foreground="#d4d4d4")
            combo.grid(row=0, column=1, sticky="e")
            combo.bind("<<ComboboxSelected>>", lambda _e, follower_index=i: self._on_follower_value_changed(follower_index))

    def _on_follower_value_changed(self, follower_index: int):
        try:
            v = int(self.follower_vars[follower_index].get())
        except ValueError:
            v = 0
        v = 0 if v < 0 else 1 if v > 1 else v

        self.table[self.selected_player][follower_index] = v

    def _on_player_select(self, _event=None):
        sel = self.player_list.curselection()
        if not sel:
            return
        self._select_player(sel[0])

    def _select_player(self, player_index: int):
        self.selected_player = player_index
        player_name = self.heroes[player_index]
        self.player_title.configure(text=f'follower Modifiers (player: "{player_name}")')

        # update follower dropdowns to match this player row
        for i in range(self.N):
            self.follower_vars[i].set(str(self.table[player_index][i]))

    def _fill_player_row(self, value: int):
    # clamp to 0/1
        value = 0 if value < 0 else 1 if value > 1 else int(value)

        a = self.selected_player
        for i in range(self.N):
            self.table[a][i] = value
            self.follower_vars[i].set(str(value))


    def _set_diagonal_2(self):
        for i in range(self.N):
            self.table[i][i] = 0
        # Refresh current player view
    ...
    def _compile(self):
        ...
        for a in range(self.N):
            player_role = HERO_ROLES.get(self.heroes[a], "damage")

            if player_role == "damage":
                target = damage_table
            elif player_role == "tank":
                target = tank_table
            else:
    ...
    def _open_hero_editor(self):
        ...
        def apply_changes():
            ...
            N = len(new_heroes)
            new_table = [[0 for _ in range(N)] for __ in range(N)]
        ...
        ttk.Label(frame, text="Tip: keep your list order stable if you rely on playerIndex * N + followerIndex.", style="Small.TLabel").grid(
